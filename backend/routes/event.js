...
router.post('/events', async (req, res, next) => {
    try {
        const { title, summary, start_day, end_day, location, capacity, category_id } = req.body;

        if (!title || !summary || !start_day || !end_day || !location || !capacity || !category_id) {
            return res.status(400).json({ error: 'All event fields including capacity are required' });
        }

        const result = await pool.query(
            'INSERT INTO Events (title, summary, start_day, end_day, location, capacity, category_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
            [title, summary, start_day, end_day, location, capacity, category_id]
        );
